...
int main() {
  ...
  window = SDL_CreateWindow("CUBES", DEFAULT_WIDTH, DEFAULT_HEIGHT,
  ...
  // set window icon
  {
    ...
    SDL_Surface* surface =
        SDL_CreateSurface(w, h, SDL_PixelFormat::SDL_PIXELFORMAT_ABGR8888);
...
#define IsMouseButtonReleased(window, button) \
  ((mouseButtons & SDL_BUTTON_MASK(button)) == false)
#define IsKeyReleased(window, key) (keys[key] == false)
    ...
    // handle inputs
    {
      ProcessButtonInput(&oldInput->uiClick, &newInput->uiClick,
                         IsMouseButtonReleased(window, SDL_BUTTON_LEFT));

      ProcessButtonInput(&oldInput->startGame, &newInput->startGame,
                         IsKeyReleased(window, SDL_SCANCODE_KP_ENTER));
      ProcessButtonInput(&oldInput->switchExitMenu, &newInput->switchExitMenu,
                         IsKeyReleased(window, SDL_SCANCODE_ESCAPE));
      ProcessButtonInput(&oldInput->switchFullscreenMode,
                         &newInput->switchFullscreenMode,
                         IsKeyReleased(window, SDL_SCANCODE_F11));

      ProcessButtonInput(&oldInput->scrollUp, &newInput->scrollUp,
      ...
      ProcessButtonInput(&oldInput->forward, &newInput->forward,
                         IsKeyReleased(window, SDL_SCANCODE_W));
      ProcessButtonInput(&oldInput->backwards, &newInput->backwards,
                         IsKeyReleased(window, SDL_SCANCODE_S));
      ProcessButtonInput(&oldInput->left, &newInput->left,
                         IsKeyReleased(window, SDL_SCANCODE_A));
      ProcessButtonInput(&oldInput->right, &newInput->right,
                         IsKeyReleased(window, SDL_SCANCODE_D));
      ProcessButtonInput(&oldInput->up, &newInput->up,
                         IsKeyReleased(window, SDL_SCANCODE_SPACE));
      ProcessButtonInput(&oldInput->down, &newInput->down,
                         IsKeyReleased(window, SDL_SCANCODE_LCTRL));

      ProcessButtonInput(&oldInput->attack, &newInput->attack,
                         IsMouseButtonReleased(window, SDL_BUTTON_LEFT));
      ProcessButtonInput(&oldInput->placeBlock, &newInput->placeBlock,
                         IsMouseButtonReleased(window, SDL_BUTTON_RIGHT));

      ProcessButtonInput(&oldInput->openInventory, &newInput->openInventory,
                         IsKeyReleased(window, SDL_SCANCODE_TAB));
      for (size_t i = 0; i < ArrayCount(oldInput->inventorySlots); i++) {
        ProcessButtonInput(&oldInput->inventorySlots[i],
                           ...
                           IsKeyReleased(window, SDL_SCANCODE_1 + i));
      }

      ProcessButtonInput(&oldInput->showDebugInfo, &newInput->showDebugInfo,
                         IsKeyReleased(window, SDL_SCANCODE_F1));

#if _DEBUG
      ProcessButtonInput(&oldInput->rebuildShaders, &newInput->rebuildShaders,
                         IsKeyReleased(window, SDL_SCANCODE_R));
      ProcessButtonInput(&oldInput->regenerateChunks,
                         &newInput->regenerateChunks,
                         IsKeyReleased(window, SDL_SCANCODE_G));
#endif
    }
