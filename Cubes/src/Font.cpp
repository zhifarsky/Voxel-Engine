...
Font *loadFont(GameMemory *memory, const char *path, float fontSize) {
  ...
  font->fileBuffer=
    readEntireFile(&memory->permStorage, path, &fileSize, FileType::binary);

  u32 atlasWidth=512, atlasHeight=512;
  u8 *tempBitmap=
    (u8 *)ArenaPush(&memory->tempStorage, atlasWidth * atlasHeight);
  ...
  font->charData=(CharData *)ArenaPushArray(&memory->permStorage,
                                            font->charsCount, stbtt_bakedchar);

  // render font to atlas
  stbtt_BakeFontBitmap(font->fileBuffer, 0, font->size, tempBitmap, atlasWidth,
                       atlasHeight, font->firstChar, font->charsCount,
                       (stbtt_bakedchar *)font->charData);

  // get font info
  stbtt_InitFont(&font->fontInfo, font->fileBuffer,
                 stbtt_GetFontOffsetForIndex(font->fileBuffer, 0));

  u8 *rgbaBitmap=
    (u8 *)ArenaPushArray(&memory->tempStorage, atlasWidth * atlasHeight, u32);
  ...
  font->atlas=Renderer::CreateTexture(
    atlasWidth, atlasHeight, rgbaBitmap, PixelFormat::RGBA, PixelFormat::RGBA,
