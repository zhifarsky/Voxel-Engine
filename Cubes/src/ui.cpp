...
void Init() {
  ...
  static Triangle faceTris[] = {Triangle(0, 1, 2), Triangle(0, 2, 3)};
...
void UI::Begin(Arena* tempStorage,
               Input* currentInput,
               Font* defaultFont,
  ...
  Renderer::setUniformMatrix4(g_UIShader, "projection",
...
// NOTE: элементы отцентрованы при помощи translate. отстальные элементы не
// отцентрованы
...
bool Button(const char* text, glm::vec2 size, bool centerX) {
  ...
  bool hovered =
      pointRectCollision(px, py, pos.x, pos.y, pos.x + size.x, pos.y + size.y);
...
bool CheckBox(const char* text, bool* value, glm::vec2 size) {
  ...
  py = displayH - py;
  bool clicked = ButtonClicked(input->uiClick);
  bool hovered = pointRectCollision(px, py, originX, originY, originX + size.x,
                                    originY + size.y);
  ...
  TextInternal(text, originX + size.x + uiStyle.margin,
               originY + FontGetSize(g_Font) / 2);
...
bool SliderInternal(const char* name,
                    const char* text,
                    float* value,
                    float minValue,
                    float maxValue,
  ...
  bool hovered =
      pointRectCollision(px, py, originX, knobPos.y - knobHeight / 2,
...

  {
    ...
    barModel = glm::translate(barModel,
                              glm::vec3(originX, originY - barHeight / 2, 0));
    barModel = glm::scale(barModel, glm::vec3(barWidth, barHeight, 1));

    glm::mat4 knobModel = glm::mat4(1.0f);
    knobModel = glm::translate(
        knobModel,
        glm::vec3(knobPos.x - knobWidth / 2, originY - knobHeight / 2, 0));
    ...
    if (hovered && clicked)
      Renderer::setUniformFloat4(g_UIShader, "color",
                                 uiStyle.sliderKnobActiveColor);
    else if (hovered)
      Renderer::setUniformFloat4(g_UIShader, "color",
                                 uiStyle.sliderKnobHoveredColor);
    else
...
float TextInternal(const char* text, float posX, float posY) {
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    Renderer::setUniformFloat2(
        ...
        -(float)(charData.y1 - charData.y0) /
            FontGetTextureAtlas(g_Font)->height);
