...
std::unordered_map<std::string, s32>
    g_UniformLocationCache[(int)ShaderAssetID::COUNT];
...
// можно при компил€ции шейдера запрашивать местоположение всех переменных и
// сохран€ть внутри шейдереа например struct Uniform {
//  const char* name;
//  s32 location;
//  bool isInitialized;
// } uniforms[16];
...
Shader GetShader(ShaderAssetID id) {
...
#endif
  {
    ...
    Shader newShader = Renderer::CreateShaderFromFile(
        g_TempStorage, asset->path, asset->shader);
...
Texture* GetTexture(TextureAssetID id) {
  ...
  {
    asset->texture =
        Renderer::CreateTextureFromFile(asset->path, PixelFormat::RGBA);
