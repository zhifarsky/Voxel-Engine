...
float g_chunkRadius=
sqrt(pow(sqrt(pow(CHUNK_SX, 2) + pow(CHUNK_SZ, 2)), 2) + pow(CHUNK_SY, 2)) /
2.0f;  // высота чанка и диагональ ширины с длиной - катеты. гипотенуза -
// диаметр сферы
...
static void setupBlockMesh(GeometryInstanced *mesh, bool staticMesh) {
  ...
  static int faceIndices[]={0, 1, 2, 2, 3, 0};
  ...
  if (firstRun) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(faceVerts), faceVerts,
                 GL_STATIC_DRAW);  // отправка вершин в пам€ть видеокарты
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(BlockFaceInstance) * MAX_FACES_COUNT,
               NULL, GL_STATIC_DRAW);

  // pos
  glEnableVertexAttribArray(1);
  glVertexAttribIPointer(1, 1, GL_UNSIGNED_SHORT, sizeof(BlockFaceInstance),
                         (void *)offsetof(BlockFaceInstance, pos));
  glVertexAttribDivisor(1,
                        1);  // перва€ переменна€ - location атрибута в шейдере
  // face direction
  glEnableVertexAttribArray(2);
  glVertexAttribIPointer(2, 1, GL_BYTE, sizeof(BlockFaceInstance),
                         (void *)offsetof(BlockFaceInstance, face));
  glVertexAttribDivisor(2,
                        1);  // перва€ переменна€ - location атрибута в шейдере
  // texture id
  glEnableVertexAttribArray(3);
  glVertexAttribIPointer(3, 1, GL_UNSIGNED_SHORT, sizeof(BlockFaceInstance),
                         (void *)offsetof(BlockFaceInstance, textureID));
  glVertexAttribDivisor(3,
                        1);  // перва€ переменна€ - location атрибута в шейдере

  // size X
  glEnableVertexAttribArray(4);
  glVertexAttribIPointer(4, 1, GL_UNSIGNED_BYTE, sizeof(BlockFaceInstance),
                         (void *)offsetof(BlockFaceInstance, sizeA));
  glVertexAttribDivisor(4, 1);
  // size z
  glEnableVertexAttribArray(5);
  glVertexAttribIPointer(5, 1, GL_UNSIGNED_BYTE, sizeof(BlockFaceInstance),
                         (void *)offsetof(BlockFaceInstance, sizeB));
...
void deleteBlockMesh(Chunk *chunk) {
  ...
  chunk->status=
    ChunkStatus::Initialized;  // TODO: в каких случа€х удал€ем буферы и какой
  // должен быть статус?
}
...
//	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(BlockFaceInstance) *
//mesh->faceCount, mesh->faces);
//
//	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
static void UpdateBlockMesh(ChunkMeshGenResult *genRes) {
  ...
  glBufferSubData(GL_ARRAY_BUFFER, 0,
                  sizeof(BlockFaceInstance) * genRes->facesCount,
                  genRes->faces);
...
bool inline IsBlockIndexValid(glm::ivec3 &index) {
  return (index.x >= 0 && index.x < CHUNK_SX && index.y >= 0 &&
          index.y < CHUNK_SY && index.z >= 0 && index.z < CHUNK_SZ);
}

bool inline IsBlockIndexValid(int y, int z, int x) {
  return (x >= 0 && x < CHUNK_SX && y >= 0 && y < CHUNK_SY && z >= 0 &&
          z < CHUNK_SZ);
}
...
void ChunkGenerateBlocks(Chunk *chunk, int posx, int posz, int seed) {
  ...
  for (s32 y=0; y < CHUNK_SY; y++) {
    for (s32 z=0; z < CHUNK_SZ; z++) {
      for (s32 x=0; x < CHUNK_SX; x++) {
        ...
        float temperature=PerlinNoise(
          seed, glm::vec2(x + posx, z + posz) * temperatureNoiseScale);
        float biomeEdge=0.5;
        ...
        // height
        float height=
          PerlinNoise(seed, glm::vec2(x + posx, z + posz) * noiseScale);
        height=(height + 1.0f) / 2.0f;
        height*=Remap(PerlinNoise(seed + 1, glm::vec2(x + posx, z + posz) *
                      (noiseScale / 5)),
                      -1, 1, 0, 1.5);
        ...
        if (y > height * 23.0f)
          ...
          float ironOre=
            PerlinNoise(seed, glm::vec3(x + posx, y, z + posz) * 30.0f);
          ironOre=(ironOre + 1.0f) / 2;
          if (ironOre > 0.7)
            block->type=BlockType::btIronOre;

          // generate caves
          float cave=PerlinNoise(
            seed, glm::vec3(x + posx, y, z + posz) * caveNoiseScale);
  ...
  for (s32 y=0; y < CHUNK_SY; y++) {
    for (s32 z=0; z < CHUNK_SZ; z++) {
      for (s32 x=0; x < CHUNK_SX; x++) {
        float tree=
          Noise(seed, glm::vec2(x + posx, z + posz) *
                treeNoiseScale);  // TODO: white nose / blue noise

        if (tree > 0.1) {
          float heightScale=PerlinNoise(
            seed + 341, glm::vec2(x + posx, z + posz) * treeHeightNoiseScale);
          ...
          for (; trunkStart.y >= 0; trunkStart.y--) {
            if (blocks[trunkStart.y][trunkStart.z][trunkStart.x].type ==
                BlockType::btGround) {
              found=true;
              break;
            }
          ...
          if (found) {
            ...
            for (; trunkY - trunkStart.y < treeHeight; trunkY++) {
              if (IsBlockIndexValid(trunkY, trunkStart.z, trunkStart.x)) {
                blocks[trunkY][trunkStart.z][trunkStart.x].type=
                  BlockType::btWood;
              }
            }
            ...
            for (int leavesY=trunkEndY;
                 leavesY - trunkEndY < treeLeavesHeight; leavesY++) {
              for (int z=-treeLeavesRadius; z <= treeLeavesRadius; z++) {
                for (int x=-treeLeavesRadius; x <= treeLeavesRadius; x++)
                  if (IsBlockIndexValid(leavesY, trunkStart.z + z,
                      trunkStart.x + x)) {
                    blocks[leavesY][trunkStart.z + z][trunkStart.x + x].type=
                      BlockType::btLeaves;
                  }
              }
...
bool IsXPosFaceVisible(Block blocks[CHUNK_SY][CHUNK_SZ][CHUNK_SX],
                       int y,
                       int z,
                       int x) {
  return x == CHUNK_SX - 1 || IsBlockTransparent(blocks[y][z][x + 1].type);
}
bool IsXNegFaceVisible(Block blocks[CHUNK_SY][CHUNK_SZ][CHUNK_SX],
                       int y,
                       int z,
                       int x) {
  return x == 0 || IsBlockTransparent(blocks[y][z][x - 1].type);
}
bool IsYPosFaceVisible(Block blocks[CHUNK_SY][CHUNK_SZ][CHUNK_SX],
                       int y,
                       int z,
                       int x) {
  return y == CHUNK_SY - 1 || IsBlockTransparent(blocks[y + 1][z][x].type);
}
bool IsYNegFaceVisible(Block blocks[CHUNK_SY][CHUNK_SZ][CHUNK_SX],
                       int y,
                       int z,
                       int x) {
  return y == 0 || IsBlockTransparent(blocks[y - 1][z][x].type);
}
bool IsZPosFaceVisible(Block blocks[CHUNK_SY][CHUNK_SZ][CHUNK_SX],
                       int y,
                       int z,
                       int x) {
  return z == CHUNK_SZ - 1 || IsBlockTransparent(blocks[y][z + 1][x].type);
}
bool IsZNegFaceVisible(Block blocks[CHUNK_SY][CHUNK_SZ][CHUNK_SX],
                       int y,
                       int z,
                       int x) {
...
ChunkMeshGenResult ChunkGenerateMesh(Arena *tempStorage, Chunk *chunk) {
  ...
  BlockFaceInstance *tempFaces=(BlockFaceInstance *)ArenaPushArray(
    tempStorage, MAX_FACES_COUNT, BlockFaceInstance);
  u32 facesCount=0;
...
#define GREEDY 1
...
#if GREEDY == 1
  bool(*usedXPos)[CHUNK_SZ][CHUNK_SX]=
    (bool(*)[CHUNK_SZ][CHUNK_SX])ArenaPushZeroArray(tempStorage, CHUNK_SIZE,
                                                    bool);
  bool(*usedXNeg)[CHUNK_SZ][CHUNK_SX]=
    (bool(*)[CHUNK_SZ][CHUNK_SX])ArenaPushZeroArray(tempStorage, CHUNK_SIZE,
                                                    bool);
  bool(*usedYPos)[CHUNK_SZ][CHUNK_SX]=
    (bool(*)[CHUNK_SZ][CHUNK_SX])ArenaPushZeroArray(tempStorage, CHUNK_SIZE,
                                                    bool);
  bool(*usedYNeg)[CHUNK_SZ][CHUNK_SX]=
    (bool(*)[CHUNK_SZ][CHUNK_SX])ArenaPushZeroArray(tempStorage, CHUNK_SIZE,
                                                    bool);
  bool(*usedZPos)[CHUNK_SZ][CHUNK_SX]=
    (bool(*)[CHUNK_SZ][CHUNK_SX])ArenaPushZeroArray(tempStorage, CHUNK_SIZE,
                                                    bool);
  bool(*usedZNeg)[CHUNK_SZ][CHUNK_SX]=
    (bool(*)[CHUNK_SZ][CHUNK_SX])ArenaPushZeroArray(tempStorage, CHUNK_SIZE,
                                                    bool);
#endif
      ...
      for (size_t x=0; x < CHUNK_SX; x+=inc) {
        ...
        if (blockType != BlockType::btAir) {
          ...
          for (Run &run : runs) {
            ...
            switch (run.face) {
              case faceXNeg:
                IsVisible=IsXNegFaceVisible;
                break;
              case faceXPos:
                IsVisible=IsXPosFaceVisible;
                break;
              case faceYNeg:
                IsVisible=IsYNegFaceVisible;
                break;
              case faceYPos:
                IsVisible=IsYPosFaceVisible;
                break;
              case faceZNeg:
                IsVisible=IsZNegFaceVisible;
                break;
              case faceZPos:
                IsVisible=IsZPosFaceVisible;
                break;
            }
            ...
            for (int i=startIndex[run.axisA]; i < chunkSize[run.axisA]; i++) {
              ...
              BlockType currentBlockType=
                blocks[accessA[0]][accessA[1]][accessA[2]].type;

              if (*currentUsed == false && currentBlockType == firstBlockType &&
                  IsVisible(blocks, accessA[0], accessA[1], accessA[2])) {
                int currentLenB=0;
                // go B
                for (int j=startIndex[run.axisB]; j < chunkSize[run.axisB];
                     j++) {
                  ...
                  bool *currentUsed=
                    &run.used[accessB[0]][accessB[1]][accessB[2]];
                  BlockType currentBlockType=
                    blocks[accessB[0]][accessB[1]][accessB[2]].type;

                  if (*currentUsed == false &&
            ...
            if (lenA > 0 && lenB > 0) {
              ...
              tempFaces[facesCount++]=
                BlockFaceInstance(blockIndex, run.face, texID, faceSize[0],
                                  faceSize[1], faceSize[2]);
            }
          }
          ...
          for (size_t i=0; i < 3; i++) {
            ...
            // neg
            if (pos[i] == 0 ||
                blocks[neighbourPosA.y][neighbourPosA.z][neighbourPosA.x]
                .type == BlockType::btAir) {
              faces[faceCount++]=BlockFaceInstance(
                blockIndex, (BlockFace)(i * 2), texID, inc, inc);
            }

            // pos
            if (pos[i] == chunkSize[i] - 1 ||
                blocks[neighbourPosB.y][neighbourPosB.z][neighbourPosB.x]
                .type == BlockType::btAir) {
              faces[faceCount++]=BlockFaceInstance(
                blockIndex, (BlockFace)(i * 2 + 1), texID, inc, inc);
            }
          }
#endif
        }
  ...
  // чтобы не выдел€ть избыток пам€ти, сначала работаем с временной пам€тью, а
  // потом копируем результат в res.faces
  res.faces=(BlockFaceInstance *)malloc(
    sizeof(BlockFaceInstance) * facesCount);  // TODO: убрать выделение в куче
...
void ChunkManagerReleaseChunks(GameMemory *memory, ChunkManager *manager) {
  // дожидаемс€ выполнени€ всех задач, чтобы потоки не использовали
  // освобожденную пам€ть
...
bool ChunkLoadFromDisk(Chunk *chunk,
...
BlockSide CubeGetSideFromRay(glm::ivec3 &cubePos,
                             glm::vec3 &rayOrigin,
                             glm::vec3 &rayDir) {
  auto RayIntersection=[](glm::vec3 &rayOrigin, glm::vec3 &rayDir, u32 plane,
                          float planeOffset) -> glm::vec3 {
    float epsilon=1e-6;
...
Block *ChunkManagerPeekBlockFromPos(ChunkManager *manager,
                                    float posX,
                                    float posY,
                                    float posZ,
                                    int *outChunkIndex) {
  ...
  for (size_t i=0; i < manager->chunksCount; i++) {
    if (posX >= manager->chunks[i].posx &&
        posX < manager->chunks[i].posx + CHUNK_SX &&
        posZ >= manager->chunks[i].posz &&
        posZ < manager->chunks[i].posz + CHUNK_SZ) {
      chunkIndex=i;
      break;
    }
  }
  ...
  glm::ivec3 relativePos(posX - manager->chunks[chunkIndex].posx, posY,
                         posZ - manager->chunks[chunkIndex].posz);

  // если блок не в пределах чанка
  if (relativePos.x >= CHUNK_SX || relativePos.y >= CHUNK_SY ||
      relativePos.z >= CHUNK_SZ || relativePos.x < 0 || relativePos.y < 0 ||
      relativePos.z < 0) {
    return NULL;
  }
  ...
  return &manager->chunks[chunkIndex]
    .blocks[relativePos.y][relativePos.z][relativePos.x];
}

// TODO: улучить алгоритм (voxel traversal
// https://www.youtube.com/watch?v=ztkh1r1ioZo)
PeekBlockResult ChunkManagerPeekBlockFromRay(ChunkManager *manager,
                                             glm::vec3 rayPos,
                                             glm::vec3 rayDir,
                                             u8 maxDist) {
  PeekBlockResult res={};
  ...
  while (dist < maxDist) {
    ...
    Block *block=ChunkManagerPeekBlockFromPos(
      manager, currentPos.x, currentPos.y, currentPos.z, &chunkIndex);
...
PlaceBlockResult ChunkManagerPlaceBlock(Arena *tempStorage,
                                        ChunkManager *manager,
                                        BlockType blockType,
                                        glm::vec3 pos,
                                        glm::vec3 direction,
  ...
  PeekBlockResult peekRes=
    ChunkManagerPeekBlockFromRay(manager, pos, direction, maxDist);
  if (peekRes.success) {
...
PlaceBlockResult ChunkManagerDestroyBlock(Arena *tempStorage,
                                          ChunkManager *manager,
                                          glm::vec3 pos,
                                          glm::vec3 direction,
                                          u8 maxDist) {
...
void ChunkManagerBuildChunks(ChunkManager *manager,
                             Arena *tempStorage,
                             Frustum *frustum,
                             float playerPosX,
                             float playerPosZ) {
  ...
  for (int z=-renderDistance; z <= renderDistance; z++) {
    for (int x=-renderDistance; x <= renderDistance; x++) {
      ...
      // sync gen
      {
        // ChunkGenerateBlocks(&manager->chunks[chunkToReplaceIndex], chunkPosX,
        // chunkPosZ, 0);
      ...
      manager->queue->AddTask(
        [saveBeforeReplacing, chunkToReplace, oldChunkPosX, oldChunkPosZ,
        chunkPosX, chunkPosZ](TaskQueue *queue, ThreadState *state) {
        // save chunk before replacing
        if (saveBeforeReplacing) {
          ChunkSaveToDisk(chunkToReplace, g_gameWorld.info.name,
                          oldChunkPosX, oldChunkPosZ);
        }
        // try loading from disk
        if (!ChunkLoadFromDisk(chunkToReplace, g_gameWorld.info.name,
            chunkPosX, chunkPosZ)) {
#if !DEBUG_CHUNKS
          // if failed, generate chunk
          ChunkGenerateBlocks(chunkToReplace, chunkPosX, chunkPosZ, 0);
#else
          ChunkGenerateBlocks_DEBUG(chunkToReplace, chunkPosX, chunkPosZ,
                                    0);
#endif
        }
