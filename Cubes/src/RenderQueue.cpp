...
/* TODO:
 * привести переменные шейдеров к одному виду, перевести рендеринг мешей на
 * RenderQueue устанавливать переменные при помощи буферов кэшировать uniform
 * location для каждого шейдера батчинг отрисовки? для ui элементов будет
 * эффективнее инстансинга
 */
...
void RenderQueuePush(RenderQueue* queue, RenderEntrySprite* command) {
  if (command->drawOnBackground)
    ...
    RenderGroupPush(&queue->mainPass, RenderEntryType::Sprite, command,
                    sizeof(*command));
}
...
void RenderSprite(RenderQueue* queue, RenderEntrySprite* entry) {
  ...
  Renderer::setUniformMatrix4(entry->shader, "view",
                              glm::value_ptr(queue->context.view));
  Renderer::setUniformMatrix4(entry->shader, "projection",
                              glm::value_ptr(queue->context.projection));

  Renderer::setUniformFloat(entry->shader, "scale", entry->scale);
  Renderer::setUniformInt(entry->shader, "spherical",
                          entry->spherical == true ? 1 : 0);
    ...
    while (i < group->mem + group->size) {
      ...
      switch (entryType) {
        ...
        case RenderEntryType::PolymeshShadowInstanced: {
          ...
          Renderer::DrawGeometryInstanced(entry->VAO, entry->triangleCount * 3,
                                          entry->instanceCount);
    ...
    while (i < group->mem + group->size) {
      ...
      switch (entryType) {
        case RenderEntryType::Polymesh: {
          ...
          Renderer::setUniformFloat3(entry->shader, "sunDir",
                                     queue->context.directLightDirection);
          Renderer::setUniformFloat3(entry->shader, "sunColor",
                                     queue->context.directLightColor);
          Renderer::setUniformFloat3(entry->shader, "ambientColor",
                                     queue->context.ambientLightColor);
...
RenderEntryMeshShadow MakeRenderEntryMeshShadow(u32 VAO,
                                                u32 triangleCount,
