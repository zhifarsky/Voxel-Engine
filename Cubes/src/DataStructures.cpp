...
void Arena::alloc(u64 capacity, u64 reserveCapacity) {
  this->capacity=
    roundToMultiple(capacity, GetMinCommitSize());  // rounding to page size
  this->size=0;
...
void *_ArenaPush(Arena *arena, u64 size, u64 alignment, bool clearToZero) {
  assert(alignment &&
         !(alignment & (alignment - 1)));  // проверка на степень двойки

  u8 *result=arena->mem + arena->size;
  s64 padding=
    -(s64)result & (alignment - 1);  // работает только со степенями двойки
