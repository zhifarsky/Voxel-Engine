...
Font* loadFont(GameMemory* memory, const char* path, float fontSize) {
	...
	u8* tempBitmap = (u8*)ArenaPush(&memory->tempStorage, atlasWidth * atlasHeight);
	...
	font->charData = (CharData*)ArenaPushArray(&memory->permStorage, font->charsCount, stbtt_bakedchar);
	...
	u8* rgbaBitmap = (u8*)ArenaPushArray(&memory->tempStorage, atlasWidth * atlasHeight, u32);
