...
void ApplyTransform(Shader shader,
                    glm::vec3 pos,
                    glm::vec3 rot,
                    glm::vec3 scale) {
...
Sprite createSprite(float scaleX, float scaleY, UV& uv) {
...

  {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * sprite.vertexCount, vertices,
                 GL_STATIC_DRAW);  // отправка вершин в память видеокарты
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 sizeof(Triangle) * sprite.triangleCount, triangles,
